...
class PlayerBus(QObject):
    ...
    # `jellytoast.player_state`; emitters that don't care can pass
    ...
    # in `jellytoast.eq_presets.BAND_FREQUENCIES` order. When `enabled`
    ...
    # `jellytoast.icons` have already refreshed their module-level
    ...
    # Unified radio presentation state. ``jellytoast.radio_state`` owns the
    ...
    # payload is a ``jellytoast.radio_state.RadioState`` (or ``None`` when
    ...
    # ── Offline / downloads ─────────────────────────────────────────────────
    # Emitted by jellytoast.offline.manager as a download moves through its
    # lifecycle, for both leaf tracks and the user-requested root node.
    # ``state`` is a ``jellytoast.offline.index.DownloadState`` value (a
    ...
    # ``jellytoast.offline.manager`` while at least one job is active or
    ...
    # Queue-level pause / resume. Fired by ``jellytoast.offline.manager`` when
    ...
    # Wi-Fi-only gate flipped. Fired by ``jellytoast.offline.manager`` when
    ...
    # ``jellytoast.offline.connectivity`` when an N-consecutive-failure
    ...
    # ── Library selection ─────────────────────────────────────────────────────
    # Fired by ``jellytoast.library_selection`` when the user changes which
    ...
    # frame (throttled to ~50 Hz) while the visualizer now-playing mode is
    # active (Settings → ``np_mode = "visualizer"``). Payload is a list of
    # floats in [0.0, 1.0] — one entry per log-spaced mel band (default 32
    # bands across 50 Hz–16 kHz). Consumed by ``VisualizerWidget``.
