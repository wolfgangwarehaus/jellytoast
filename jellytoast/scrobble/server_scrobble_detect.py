...
``jellytoast/scrobble/listenbrainz.py``), Navidrome stamps ``"navidrome"``. So if
...
# jellytoast/scrobble/listenbrainz.py. Anything else in the user's listens is a
# DIFFERENT scrobbler.
...
def detect(
    ...
    *,
    ...
    caller dispatches via ``jellytoast.async_io.run_async``.
