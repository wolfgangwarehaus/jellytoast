...
See jellytoast/theme.body_color_for() and docs/research/portable_blur.md.
...
Wayfire, SwayFX), the user can target jellytoast in their own window rules:
our Wayland app_id is the stable string "jellytoast" (set via
setDesktopFileName).
"""
...
def apply(
    ...
    if enabled and opaque_mode_active():
        ...
        enabled = False
    # macOS: native NSVisualEffectView vibrancy is live again via the
    # sibling-below backend (blur/_macos.py) — the effect view rides below Qt's
    # content view instead of swapping the content view, which fixes the
    # blank/mis-drawn window on resize / activation that disabled the earlier
    # content-view-swap path.
