...
thread via ``jellytoast.async_io.run_async``.
...
- **Cast-proxy first.** Every URL we push to a speaker is routed
  through ``jellytoast.cast_proxy.resolve_cast_url`` so a Tailscale /
...
The package layout mirrors ``jellytoast/scrobble/``: this is the public
