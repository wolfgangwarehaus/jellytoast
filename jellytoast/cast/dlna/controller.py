...
``jellytoast.cast.dlna``; nothing depends back on it.
...
(``jellytoast.cast.dlna``) at call time rather than imported directly.
...
def _lan_search_sources() -> list:
    ...
    try:
        from jellytoast.cast_manager import _discovery_interfaces
...
def _ensure_async_upnp() -> bool:
    ...
    The test suite patches ``jellytoast.cast.dlna._ensure_async_upnp``;
...
class DlnaController:
    ...
    def play(
        ...
        *,
        ...
        ``jellytoast.cast_proxy.resolve_cast_url`` before reaching this
