...
``jellytoast/cast/dlna.py`` monolith. Public surface is preserved
verbatim: ``cast_manager.py`` and every other importer continues to
``from jellytoast.cast.dlna import …`` unchanged, and the test suite's
...
- **Private asyncio loop on its own worker thread**.  jellytoast uses no
  asyncio elsewhere; ``jellytoast/async_io.run_async`` (Qt thread pool) is
...
- **Cast-proxy is mandatory, not opt-in**.  DLNA renderers fetch only
  ...
  ``jellytoast.cast_proxy.resolve_cast_url``, same as Chromecast +
