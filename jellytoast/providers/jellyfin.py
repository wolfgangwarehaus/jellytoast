...
declared in ``base``. Existing callers of ``jellytoast.jellyfin_api.get_api()``
...
from jellytoast.jellyfin_api import JellyfinAPI, get_api
from jellytoast.providers.base import AuthResult, MediaProvider, ServerInfo
...
class JellyfinProvider(MediaProvider):
    ...
    def query_items(self, rules: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        * ``match="any"`` — each rule is evaluated independently and
        ...
        ``SortName``.
        """
        from jellytoast.providers.smart_rule_eval import matches_rule, refine_items
    ...
    def _fetch_paged(
        ...
        *,
        ...
        rule)."""
        from jellytoast.providers.smart_rule_eval import _field_value
