...
- **Audio output device** (setting `playback/audio_output_device`, default `auto`): Settings → Playback → Audio output pins mpv's `--audio-device` to any enumerated output — PipeWire/Pulse sinks, WASAPI endpoints, or a raw ALSA `hw:` device (the direct, PipeWire-bypassing audiophile path). Applies on the next track; falls back to Auto if the pinned device won't open. Raw ALSA suppresses crossfade (exclusive — one open at a time) and the visualizer paints an explanatory caption (no PipeWire stream to tap). See `docs/research/audio_output_routing.md`.
...
- **Connectivity tracker:** `is_server_reachable` is driven by API-call outcomes — only network-class failures (`RequestException`, timeout) count; HTTP 4xx/5xx leave reachability alone since the server *is* reachable. Two consecutive failures flip the state to unreachable — but only once a minimum elapsed window (~4 s) has passed since the first failure of the run, so a near-simultaneous burst (e.g. a library grid fanning ~8 requests at once) can't spuriously flip the app offline. A single success clears the counter and flips it back. State transitions emit `connectivity_changed(bool)` on `PlayerBus`.
