"""Tests for the bundled app-icon loader (modules.ui_helpers).

The brand mark must ship inside the wheel: it's loaded via
``importlib.resources`` from ``modules.assets`` so an installed build
...
def test_icon_resource_resolves_in_package():
    ...
    res = ir.files("modules.assets").joinpath("jellytoast.svg")
    assert res.is_file()
    data = res.read_bytes()
