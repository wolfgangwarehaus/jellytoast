...
class TestResyncNoOp:
    ...
    def test_identical_metadata_no_changes(self, offline_db, install_provider):
        meta = {"Id": "t1", "Name": "Track", "RunTimeTicks": 1000,
...
class TestResyncBlobDrift:
    def test_date_modified_change_marks_stale(self, offline_db,
                                              install_provider):
    ...
    def test_runtime_ticks_change_marks_stale(self, offline_db,
                                              install_provider):
        _seed("t1", {"Id": "t1", "Name": "T", "RunTimeTicks": 1000})
        install_provider(_FakeProvider({
            "t1": {"Id": "t1", "Name": "T", "RunTimeTicks": 2000}
        }))
        out = _snap.resync("t1")
        assert out["marked_stale"] is True

    def test_blob_drift_only_marks_complete_nodes(self, offline_db,
                                                  install_provider):
...
class TestResyncDeleted:
    def test_missing_item_flags_deleted_server_side(self, offline_db,
                                                    install_provider):
        _seed("t1", {"Id": "t1", "Name": "T"})
    ...
    def test_missing_item_marks_complete_node_stale(self, offline_db,
                                                    install_provider):
    ...
    def test_missing_item_does_not_delete_blob(self, offline_db,
                                               install_provider):
...
class TestResyncErrors:
    def test_provider_exception_surfaces_in_error(self, offline_db,
                                                  install_provider):
        _seed("t1", {"Id": "t1", "Name": "T"})
...
class TestIsStale:
    def test_returns_false_for_missing_node(self, offline_db,
                                            install_provider):
        install_provider(_FakeProvider())
